import { pgTable, varchar, uuid, boolean, text, timestamp, integer, jsonb, index, uniqueIndex, primaryKey, pgEnum } from 'drizzle-orm/pg-core';
...
export const permissionGroupItemsTable = pgTable('permission_group_items', {
  permissionGroupId: uuid('permission_group_id').notNull().references(() => permissionGroupsTable.id, { onDelete: 'cascade' }),
...
export const userRolesTable = pgTable('user_roles', {
  ...
  departmentId: uuid('department_id').references(() => departmentsTable.id, { onDelete: 'cascade' }),
...
export const approvalRightsTable = pgTable('approval_rights', {
  id: idColumn(),
  tenantId: uuid('tenant_id').notNull().references(() => tenantsTable.id, { onDelete: 'cascade' }),
  roleId: uuid('role_id').references(() => rolesTable.id, { onDelete: 'cascade' }),
  userId: uuid('user_id').references(() => usersTable.id, { onDelete: 'cascade' }),
...
export const delegationsTable = pgTable('delegations', {
  id: idColumn(),
  tenantId: uuid('tenant_id').notNull().references(() => tenantsTable.id, { onDelete: 'cascade' }),
  delegatorUserId: uuid('delegator_user_id').notNull().references(() => usersTable.id, { onDelete: 'cascade' }),
  delegateUserId: uuid('delegate_user_id').notNull().references(() => usersTable.id, { onDelete: 'cascade' }),
  roleId: uuid('role_id').references(() => rolesTable.id, { onDelete: 'cascade' }),
