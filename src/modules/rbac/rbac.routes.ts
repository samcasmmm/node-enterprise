...
import { createRoleSchema, updateRoleSchema, setPermissionsSchema, assignRoleSchema } from './rbac.validation.js';
...
const permissionController = container.resolve<PermissionController>(TOKENS.PermissionController as any);
...
router.use(
  ...
  buildCrudRouter(roleController, {
    ...
    extend: (r) => {
      r.put('/:id/permissions', isAuth, requirePermission('rbac:update'), validate(setPermissionsSchema as any), roleController.setPermissions);
      r.post('/:id/assign', isAuth, requirePermission('rbac:update'), validate(assignRoleSchema as any), roleController.assignToUser);
    },
  }),
);
