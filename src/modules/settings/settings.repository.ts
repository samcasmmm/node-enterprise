...
export class SettingsRepository extends BaseRepository<typeof settingsTable, Setting, NewSetting> {
  ...
  async findByCategory(tenantId: number, category: string): Promise<Setting[]> {
    return db
      .select()
      .from(settingsTable)
      .where(and(eq(settingsTable.tenantId, tenantId), eq(settingsTable.category, category)));
  }
  ...
  async upsert(
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(settingsTable.tenantId, tenantId),
    ...
    const [row] = await db
      ...
      .values({ tenantId, category, key, value, isSecret })
      .returning();
    return row;
  }
}
